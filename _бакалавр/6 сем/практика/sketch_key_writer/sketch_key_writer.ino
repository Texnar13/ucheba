  ...
  if (ibutton.search(addr)) {  // запускаем поиск dallas
    ...
    if (!writeflag){ 
      ...
      if (readflag) {
        ...
        // пытамся считать внутреннюю память посылая команды чтения
        if(tryConnectDS199xL(keyID, ibutton)){
          // память читается это ключ DS1995L
          out_titleString("DS1995L");out_redraw();
          Serial.println("DS1995L -> Dump!");
          // получить дамп памяти (и вывести его в serial)
          dumpDS1995L(keyID, ibutton);
        }
...
void ACsetOn(){
  ...
  ADMUX = 0b00000011;        // подключаем к AC Линию A3
}
